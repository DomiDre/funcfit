...
export class FittingService {
  ...
  run_fit() {
    ...
    const p_init: Float64Array = new Float64Array(this.selectedModel.parameters.length);
    const vary_p: Uint8Array = new Uint8Array(this.selectedModel.parameters.length);
    ...
    const p_result: Parameter[] = [];
    for (const idx in p_params) {
      const param = this.selectedModel.parameters[idx];
